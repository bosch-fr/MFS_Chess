/*********************************************************************************************/
/* A chess program smaller than 2KB (of non-blank source), by H.G. Muller, 2006 Netherlands  */
/* Micro-Max, version 4.8                                                                    */
/* Ported to Arduino MEGA (CHESSuino), LCD display, by Diego Cueva, June 2014 Equator        */ 
/* Ported to Atmel ATMega644, 7seg display, by Andre Adrian, 2019                            */
/* Ported to Arduino UNO by rom3, console display, May 12 2019 Germany                       */
/* Ported to 'Multi Function Shield' for Arduino , 7seg display, by C. Bosch May 2025 France */
/*********************************************************************************************/
...
long  N;                     /* N=evaluated positions+S */
long T = 0x3F;               /* T=recursion limit  (Computer Play strength) default 3F */
...
char const    TabCarAlp[8] = {97,98,99,100,101,102,103,104};
char const    TabCarNum[8] = {49,50,51,52,53,54,55,56};
bool GetBut = false;
byte buttonNumber = 0; 
byte buttonAction = 0;
byte btn = 0;
int af = 0; // 1 to 4 // 7seg index
int aff = 0;
int ia = 0; // 1 to 8 // TabCarAlp index
int in = 0; // 1 to 8 // TabCarNum index
bool r0 = false;
bool r2 = false;
bool r3 = false;
...
void setup() {
  ...
  MFS.setDisplayBrightness(3); // 0 = max, 3 = min
...
void getMenu() // switch between PLAY/ELO, and set ELO level
  {
  MFS.write("PLAY");  
  while (r0==false)
    {   
...
void getButton() 
  ...
  while (aff<4)
    {
    while ((r0==true)&&(r3==false))
      {
      if (aff==0)
      {if (r2==true) {btn = MFS.getButton();} else {r2=true;}}
      if (btn)
        {
        buttonNumber = btn & B00111111; buttonAction = btn & B11000000;
        if ((buttonAction == BUTTON_PRESSED_IND)&&(buttonNumber==2)||((buttonNumber==3))) 
          { // 'VALID' button
          if ((aff==0) || (aff==2)) {inChar = TabCarAlp[ia];} else {inChar = TabCarNum[in];}
          inputString += inChar; MFS.write(inputString.c_str());
          while (af==aff) // cycle through alpha table
            {
            btn = MFS.getButton();// wait for 'SELECT' or 'VALID' button
            if (btn)
              {
              buttonNumber = btn & B00111111; buttonAction = btn & B11000000;
              if ((buttonNumber==2) && (buttonAction == BUTTON_PRESSED_IND)) 
                { // 'VALID' button
                ia=0; in=0; r0=false;
                if (aff==3){af=0;stringComplete=true;}else{af++;}
                }
              if ((buttonNumber==1) && (buttonAction == BUTTON_PRESSED_IND)) 
                { // 'SELECT' button
                inputString.remove(af,1);
                if ((aff==0) || (aff==2)) 
                {if (ia>=7) {ia=0;} else {ia++;}
                  inChar = TabCarAlp[ia];}
                else
                {if (in>=7) {in=0;} else {in++;}
                  inChar = TabCarNum[in];}
                inputString += inChar; MFS.write(inputString.c_str());
                }
              if (aff!=0) { 
                if ((buttonNumber==3) && (buttonAction == BUTTON_PRESSED_IND)) 
                  { // 'CLEAR' button
                  af=0; ia=0; in=0; r0=false; r3=true;
                  inputString = "";
                  }
                }
              }
